...
## Firestore readiness (Phase 12)

Catalog collections remain **read-only** for clients. RFQ/quote catalog snapshot and match fields persist inside embedded `items[]` on `quoteRequests` and `supplierQuotes` (app-validated). Indexes in `firestore.indexes.json` cover catalog variant browse/search and quote list queries.

See `CATALOG_SEARCH_FOUNDATION.md` — Firestore security section.
